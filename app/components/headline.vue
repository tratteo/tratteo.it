<template>
    <div class="drawer-end sticky top-0 z-[10000] w-full max-w-[var(--content-width)] h-[var(--headline-height)]">
        ...
        <div class="drawer-side z-[10000]">
            ...
            <nav class="min-h-full bg-default w-64 pt-4">
                <ul class="menu">
                    <li>
                        <NuxtLink to="/">
        ...
        <!-- Navbar menu -->
        <div
            class="drawer-content h-full flex flex-col transition-all duration-100 ease-out rounded-lg border-base-content/20 px-2 mt-2"
            :class="transparentBar ? ['bg-transparent'] : ['shadow-md', 'border', 'bg-default']"
        >
                        ...
                        <li v-for="sec in sections">
                            ...
                            <details v-else class="p-0 nav-dropdown">
                                ...
                                <ul class="relative text-start bg-transparent flex flex-col items-stretch text-base-content">
                                    <div
                                        class="z-[1] absolute top-0 bottom-0 left-0 right-0 bg-default border-base-content/20 border rounded-btn opacity-50 pointer-events-none"
                                    ></div>
...
function updateBarTransparency() {
    const progress = y.value / (document.documentElement.scrollHeight - document.documentElement.clientHeight);
...
function focusNavigationSection(id: string | undefined) {
    document.querySelector(`section#${id}[navigation]`)?.scrollIntoView();
}
onMounted(() => {
